...
# thus, null hypothesis assumes normality; if p is significant, less than 0.05 you reject the null and say that the data follows a non-normal distribution

shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_040"])
# p value less than 0.05 so reject the null hypothesis that data follows a normal distribution
...
# these are the only 2 sites that follow a normal distribution
...
#results: p significant so reject null that variances are not equal
...
# df = no. of factors - 1
# sum sq = deviation of each observation from mean
# mean sq = sum sq/df

#results: reject null hypothesis i.e. difference between a pair of group means is statistically significant
# difference in means not significant; statistically might as well be the same
# so if we reject, it is statistically significant that they ARE different, so treat them as different
# so, the means are NOT all the same, some are different
# but which pair of means are different, we don't know yet

# Running ANOVA using lm
Litter.Totals.anova2 <- lm(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova2)
# first site = 040 is written as the intercept in the output table
...
# to identify which pair of means are not equal
# TukeyHSD() computes Tukey Honest Significant Differences
TukeyHSD(Litter.Totals.anova)
# diff = difference of means; then lower & upper end points of confidence intervals, and p-values

# Extract groupings for pairwise relationships
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE)
# finds groups that have similar means; groupings for pairwise relationships
Litter.Totals.groups
# sites with the same litter do not have a significant difference
...
> Reorder sites, same order as groups list
...
# * shows interaction within variables
# if p value less than 0.05, interaction is significant
...
# new object w/ 24 levels resulting from functional group & class
# and each row of litter, one of the 24 levels is assigned

...
#Returns highest/max value in the vector
max(vector1)

#Create another vector of the values 10 thru 15
vector4 <- c(10:15)

#Needed to create a vector
c(vector1, vector4)

#Goes down the line and multiples individual values within the two vectors by each other
vector1 * vector4

#Multiples all values in the vector by 10
vector1 * 10

#Converts to uppercase
toupper(vector2) 

#Inverts values in the vector (! means not)
!vector3
...
#goes as row1,column1 - row2,column1...

matrix2 <- matrix(101:120, nrow = 5, ncol = 4, byrow = TRUE) #
matrix2
#matrix2 has 5 rows and 4 columns
#just the order changed with row1,column1 - row1,column2
...
matrix3
#dimnames = dimension names, assigns values/labels to the rows and columns of the matrix we created
#row names first, column names second
```

### Matrix Operations
Add comments in the code chunk below to indicate type of vectors created and what each operation does.
```{r Matrix Operations}
#why [], not (); is it because we're retrieving, not assigning
matrix1[4, 1] #returns value for row4,column1

matrix1[4,  ] #returns all values for row4

matrix1[ , 3] #returns all values for column3

matrix1[c(12, 14)] #

matrix1[c(12:14)] #

matrix1[2:4, 1:3] #all data in row 2-4, columns 1-3
...
matrix4 <- matrix(
  ...
  dimnames = list(
    rnames, 
    cnames)
  ) # established values for rows and columns previously
...
#Reverts back entire list
list1

#Reverts back age values
list1$age
...
#Make sure it's same length as the pre-existing columns
dataframe1$month <- c('Jan','Feb', 'Mar', 'Apr')
#matrix vs list vs dataframe?
```

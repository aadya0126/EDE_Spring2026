...
#variable y (dependent) ~ variable x (independent)
...
# Results: residuals & coefficients (shows positive or negtive relationship)
# p-value = confidence level
# r squared = how much does depth explain irradiance
...
# found a strong negative correlation = 0.55
# p-value significant
```
...
# c(2,2) for a 2x2 grid and c(4,4,4,4) for the size of the plots
...
# eliminating zeroes and any irradiance values greater than 5000
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth)
summary(irradiance.regression2)
# after log transformation, r squared increased by a lot = a much better fit; p value still significant
...
# phosphorus (y) dependent on both depth & oxygen (x values)
summary(TPregression)
# degrees of freedom reduce if the sample size is smaller & if number of variables increases
# significant p values for both variables
# low r squared value = 29.85% explained by depth & oxygen
...
```{r, fig.height = 3, fig.width = 4}
# r-squared told us more variables could be at play, we want to find relevant variables -> creating a correlation matrix
...
# oval shapes are kind of representing scatter plots between the 2 variables -> thinner the plot, higher the correlation (can be approximated by a straight line) 
# color from dark red (strong -ve) to dark blue (strong +ve)
# lines mean same variable, 1.0 correlation
corrplot.mixed(PeterPaulCorr, upper = "ellipse")
# showing upper diagonal only so you can see both ellipse and values on the same plot
...
```{r}
# we want a model with a high r-squared but not too many variables; find balance through AIC
# smaller AIC better
...
step(TPAIC)
# starting result will show all variables
# then new AIC when 1 variable is removed
# best regression found to be using the combo of oxygen, temp, & N

TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug)
summary(TPmodel)
# but r-square ended up being worse than just depth & temp lol

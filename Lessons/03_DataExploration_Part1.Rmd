...
> Answer: Console shows if loading a package was successful or not. If loads fine, no message. If there is an error it'll show in the console or if 2 packages have the same function it'll show as a "mask"
...
# What happens if we don't assign a name to our imported dataset? Multiple rows were omitted?
read.csv(here("./Data/Raw/USGS_Site02085000_Flow_Raw.csv"),stringsAsFactors = TRUE)
#./ <- 1 dot is this folder
# ../<- 2 dots is the folder above or outside of this folder

#If you write read.csv("") and click "tab" between the quotations, you will find files to choose from. However, you won't get the path.
#To find the path, do the same in the console which will give you the outermost file option from which you can start making your way in. Copy that path for your code here. 
```
...
> ANSWER: File in data folder -> Metadata -> README_USGS_Site
...
```{r Exploring data}
#View(USGS.flow.data) #should comment out view & install.packages before knitting
...
str(USGS.flow.data) #str = structure
dim(USGS.flow.data) #dim = rows & columns
length(USGS.flow.data) #how many columns

head(USGS.flow.data) # first 5-6 records into console
head(USGS.flow.data, 10) # for first 10 records
tail(USGS.flow.data, 5) # for last 5 records
USGS.flow.data[30000:30005, c(3, 8, 14)] # a slice of rows from 30000-30005, with columns 3,8,and 14

class(USGS.flow.data$datetime) # $ sign identifies a specific column
# column date/time shows up as a factor class - not what we want
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data) #shows summary stats of all columns
#can point to individual columns with $

```

What happened to blank cells in the spreadsheet when they were imported into R?
> Answer: Showed up as NA
...
#NA shows up for numeric columns. For character columns it's just blank

```{r Finding NAs}
#summaries for individual columns
...
```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #removed NAs from any row, any column
dim(USGS.flow.data)
dim(USGS.flow.data.complete)
# now values down to 5342 rows w/ 15 columns

mean(USGS.flow.data.complete$discharge.mean)
#mean for the original file w/ NA values resulted in NA as the output because we can't calculate mean w/ missing values
sd(USGS.flow.data.complete$discharge.mean)
summary(USGS.flow.data.complete$discharge.mean)
# maybe in using omit we might have removed rows which had data for one column but not for another -> very crude way of removing NAs
```
...
str(USGS.flow.data)
# site numbers shows up as integers because they're numbers BUT these should be treated as characters e.g. a value of 080 would be treated as 80 if an integer but as 080 if treated as a character
```
...
```{r}
USGS.flow.data$site_no <- as.factor(USGS.flow.data$site_no) #data type changed to a factor type instead of integer; values remain same but are now characters
str(USGS.flow.data)
```
...
USGS.flow.data2  <- read.csv(
  ...
  ) #read that specific column in as a factor
class(USGS.flow.data2$site_no)
```
...
%m  month as number (00-12, can be e.g., 01 or 1)
%y  2-digit year
%Y  4-digit year
...
#as.date converts string object into a date object
# so it doesn't follow mdy? Why is it ymd?
...
```{r Fixing dates}
# converting datetime back into a character/string column
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")
class(USGS.flow.data$datetime)
# create a function: is the value less than 191226 (because that is the cutoff where the mistakes are happening), if yes add 19, if no add 20
create.early.dates <- (function(d) {
       paste0(ifelse(d < 191226,"19","20"),d)
       })
help("paste0")
help("ifelse")
# assigned the function to the specific column we want to change
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)
USGS.flow.data$datetime[1]

# change column back to date/time format
...
write.csv(
  ...
  row.names=FALSE)
#row.names false because we're not labeling the rows

...
today <- Sys.Date() #standard date format is y-m-d
format(today, format = "%B") #converting specifically to unabbreviated month
format(today, format = "%A") #converting specifically to unabbreviated day of the week
#format(today, format = "")
#format(today, format = "")

#importing dataframe, dates seen as string
...
str_juneteenth2 <- paste("19 June","the_year") #paste combines different strings together into one
date_juneteenth2 <- dmy(str_juneteenth2) #CHECK LECTURE RECORDING
```
### The century issue
...
fix.early.dates <- function(d, cutoff) {
       ...
       m <- year(d) %% 100  
       # The %% operator does modular division i.e. integer-divides #so %% 100 returns last 2 values 
...
date_obj_past
#26L = integer; integers have no room for decimals but numbers do
...
# want to treat character columns as categories; factor = type of character
# factor w/ 133 levels = 133 unique values
dim(sites)

#Convert date column to date object
sites$date <- ymd_hms(sites$date)
#hms - hour minutes seconds ; POSIXct shows up in the output df w/ standard date formats
...
#Convert to a factor; treating days as categories is easier
...
levels(sites$dow) #levels = unique values you see
```
...
#using the commas and '' combines the values
...
#Factors have levels and labels
```

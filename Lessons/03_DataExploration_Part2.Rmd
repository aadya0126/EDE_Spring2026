...
# 3. Import datasets w/ strings and site_no showing up as factors
USGS.flow.data <- read.csv(
  file = here('Data/Processed/USGS_Site02085000_Flow_Processed.csv'),
  stringsAsFactors = TRUE,
  colClasses = c('site_no'='factor')
)
#characters vs factors (grouping variables rather than raw data)
...
#using dashes because original file has dashes not slashes
class(USGS.flow.data$datetime)
...
ggplot(data = USGS.flow.data, aes(x = discharge.mean.approval)) +
  geom_bar()
# ggplot(value from data, aesthetics (x from this column)) + type of plot
# using gear/option button, can move chunk output in console for a better view
# y shows up as no. of rows/records
```
...
```{r, fig.height = 3, fig.width = 4}
# Creating histogram w/ assigned x
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))
# aes can be in geom or in ggplot

# Width of individual bars/bins will span 10 units of mean discharge
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# No. of bins will be 20 units of mean discharge
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# Bin width is 10 units and x axis will now go from 0 to 500
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
# Looking at a different column for x axis
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
# fig height & width for knitted pdf outputs

# frequency line graph showing mean height of the streams as x axis
# adding another layer of graph on one paper (not easy to do w/ histogram); changed color to show difference in data
# adding a third layer; changed line type to dashes
# look at Help dropdown -> Cheat Sheets -> Data Visualization w/ GGPlot2
# restricted x axis from 0 to 10
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean), bins = 50) +
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "red") +
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 3) +
  scale_x_continuous(limits = c(0, 10))

# set color inside aesthetics, meaning the category assigned will get a unique color
# adding a legend w/ position as top of the graph
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean, color = gage.height.mean.approval), bins = 50) +
  scale_x_continuous(limits = c(0, 10)) +
  theme(legend.position = "top")
# messages w/ removed rows = missing data
```
...
# every point is a row in a dataframe; values showing an increasing x-y relationship

ggplot(USGS.flow.data) +
  geom_point(aes(x = datetime, y = discharge.mean))
# seeing gaps in dataset
```
